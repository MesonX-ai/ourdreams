...
env. It mirrors `out/` → `public_html` and `php/` → `public_html/api`, copies `.htaccess` to
the web root, and **never touches `wp/`**. FTP credentials are never committed (see
`.gitignore` and the security note below).
