( function( $ ) {
	...
	function ourdreamsAnimeAnimation( target, options ) {
	...
	/****** start fancy text  ******/
	function OurDreamsFancyTextDefault( item, ftOptions ) {
		...
		if ( fancy_el.length > 0 ) {
			fancy_el.forEach( element => {
				...
				if ( typeof ( fancy_options ) !== 'undefined' && fancy_options !== null ) {
					...
					let child = element;
					OurDreamsFancyTextDefault( child, fancy_options );
		...
		if ( anime_el.length > 0 ) {
			anime_el.forEach( element => {
				...
				if ( 'undefined' !== typeof ( options ) && null !== options ) {
					...
					ourdreamsAnimeAnimation( element, options );
			...
			if ( ! $( element ).hasClass( 'appear' ) ) {
				if ( elementorFrontend.isEditMode() ) {
					...
					fancyInterval = setTimeout( function() {
						OurDreamsFancyTextDefault( element, ftOptions );
						$( element ).addClass( 'appear' );
					}, 1000 );
				} else {
					OurDreamsFancyTextDefault( element, ftOptions );
	...
	$.fn.fancyTextEffect = function() {
		...
		if ( $( text_rotator ).length > 0 ) {
			$( text_rotator ).each( function() {
				...
				if ( $( '.elementor-widget-ourdreams-text-rotator' ).length > 0 ) {
