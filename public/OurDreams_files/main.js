( function( $ ) {
	...
	$document.ready( function() {
		...
		el_body.addClass( 'ourdreams-theme-ready' );
		...
		$document.on( 'touchstart click', 'body', function( e ) {
			if ( ! ( $( e.target ).closest( '.navbar-nav' ).length || $( e.target ).closest( '.navbar-full-screen-menu-inner' ).length ) && ! $( '.ourdreams-left-menu-wrap' ).length ) {
			...
			if ( ! el_navbar_collapse.has( e.target ).is( '.navbar-collapse' ) &&
				...
				! el_navbar_collapse.hasClass( 'ourdreams-left-menu-wrapper' ) ) {

				el_navbar_collapse.collapse( 'hide' );
			}
		});

		var navbarWidgetNavbar       = $( '.header-common-wrapper.standard .elementor-widget-ourdreams-mega-menu .navbar-collapse' );
		var navbarWidgetNavbarToggle = $( '.header-common-wrapper.standard .elementor-widget-ourdreams-mega-menu .navbar-toggler' );
		...
		function mobileModernFullscreenNavigation() {
			...
			if ( getWindowWidth() <= tabletBreakPoint ) {
				if ( ( mobileNavStyle == 'modern' || mobileNavStyle == 'full-screen-menu' ) && ! $( '.navbar-' + mobileNavStyle + '-inner' ).length ) {
					...
					if ( $( '.navbar-collapse-clone' ).length > 0 && 'undefined' != typeof mCustomScrollbar && 'undefined' != typeof OurDreamsMain && $.inArray( 'mCustomScrollbar', OurDreamsMain.disable_scripts ) < 0 ) {
		...
		$window.on( 'resize', function() {
			...
			if ( ! el_body.hasClass( 'elementor-editor-active' ) ) {
				if ( 'undefined' != typeof OurDreamsMain &&
					...
					$.inArray( 'isotope', OurDreamsMain.disable_scripts ) < 0 ) {
		...
		function setHeaderTopSpace() {
			...
			var pageContent  = $( '.ourdreams-main-content-wrap' ).find( '.entry-content-inner' ),
				sectionFirst = pageContent.find( '.elementor .e-con' ).first();

			// for page title first section
			var pageTitle        = $( '.ourdreams-main-title-wrapper' ),
			...
			if ( pageTitle.length > 0 && pageTitlesection.hasClass( 'top-space' ) || $( '.default-main-title-wrapper' ).length > 0 ) {
			...
			} else if ( $( '.ourdreams-main-inner-content-wrap' ).hasClass( 'top-space' ) && 0 === pageTitle.length ) {
				// single post
				$( '.ourdreams-main-inner-content-wrap.top-space' ).parents( '.ourdreams-main-content-wrap' ).css( 'margin-top', ts_height + 'px' );
				...
				$( '.ourdreams-main-content-wrap .top-space, .error-404.top-space' ).parents( '.ourdreams-main-content-wrap' ).css( 'margin-top', ts_height + 'px' );

			} else if ( $( '.ourdreams-single-product-main-wrap' ).hasClass( 'top-space' ) ) {
				// Single Product Top Space
				$( '.ourdreams-single-product-main-wrap.top-space' ).css( 'margin-top', ts_height + 'px' );

			} else if ( $( '.ourdreams-main-content-wrap' ).hasClass( 'top-space' ) ) {
				// Archive Product Top Space
				$( '.ourdreams-main-content-wrap.top-space' ).css( 'margin-top', ts_height + 'px' );

			} else {
				if ( 'undefined' != typeof OurDreamsMain && '1' == OurDreamsMain.mobile_header_top_space ) {
			...
			const $elPageTitleWrap = $( '.ourdreams-main-title-wrap' );
		...
		function fullScreenHeight() {
			...
			if ( 'undefined' != typeof OurDreamsMain && $.inArray( 'imagesloaded', OurDreamsMain.disable_scripts ) < 0 ) {
				const $pageLayout = fullScreenObj.parents( '.ourdreams-page-layout' );
